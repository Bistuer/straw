...
public interface ItemRepository extends ElasticsearchRepository<Item, Long> {
    ...
    //按两个条件查询并按指定字段排序
    Iterable<Item> queryItemsByTitleMatchesAndBrandMatchesOrderByPriceDesc(String title, String brand);

    //实行分页查询
    //返回值不再是Iterable而变为了Page
    Page<Item> queryItemsByTitleMatches(String title, Pageable pageable);
